...
if ($data_type == 'GET_USERID_FOR_TOWERS') {
    ...
    for ($i = 0; $i < count($res); $i++) {
        ...
        if ($SLA_name == 'Telenor') {
            ...
            $agregation = '<select name="Agregation" id="AGR' .  $safeID . '" class="form-control search-select" required>';
    ...
    for ($i = 0; $i < count($res); $i++) {
        ...
        $safeID = htmlspecialchars($res[$i]['ID'], ENT_QUOTES, 'UTF-8');
        $result[$i][5] = '<select class="form-control" name="outage_type" id="outage_type' . $safeID . '">'; //ADDED BY ANUM
        ...
        $result[$i][6] = '<select class="form-control" name="issue_reported_by" id="issue_reported_by' . $safeID . '">';
